"""GET/POST/PUT/DELETE /api/v1/hiring_stage_templates — reusable stages + focus attributes."""
...
from app.services.hiring_stage_template_service import HiringStageTemplateService


class HiringStageTemplatesController(BaseController, Authenticatable):
    @before_action
    ...
    def index(self):
        ...
        return self.render_json(HiringStageTemplateService(self.db).list(account_id)["data"])

    def show(self):
        account_id = self._account_id()
        self.require_permission("hiring_structure", "view", account_id=account_id)
        tid = int(self.request.path_params["id"])
        r = HiringStageTemplateService(self.db).get(account_id, tid)
        if not r["ok"]:
    ...
    async def create(self):
        ...
        r = HiringStageTemplateService(self.db).create(account_id, body if isinstance(body, dict) else {})
        if not r["ok"]:
    ...
    async def update(self):
        ...
        tid = int(self.request.path_params["id"])
        body = await self._get_body_json()
        r = HiringStageTemplateService(self.db).update(account_id, tid, body if isinstance(body, dict) else {})
        if not r["ok"]:
            return self.render_error(r["error"], 422)
        return self.render_json(r["data"])
    ...
    def destroy(self):
        ...
        self.require_permission("hiring_structure", "manage", account_id=account_id)
        tid = int(self.request.path_params["id"])
        r = HiringStageTemplateService(self.db).destroy(account_id, tid)
        if not r["ok"]:
            return self.render_error(r["error"], 404)
        return self.render_json(r["data"])
