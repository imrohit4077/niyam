import type { TrendDirection } from './dashboardUtils'

const icons = {
  users: (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
      <circle cx="9" cy="7" r="4" />
      <path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" />
    </svg>
  ),
  briefcase: (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <rect x="2" y="7" width="20" height="14" rx="2" ry="2" />
      <path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16" />
    </svg>
  ),
  calendar: (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
      <line x1="16" y1="2" x2="16" y2="6" />
      <line x1="8" y1="2" x2="8" y2="6" />
      <line x1="3" y1="10" x2="21" y2="10" />
    </svg>
  ),
  gift: (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <polyline points="20 12 20 22 4 22 4 12" />
      <rect x="2" y="7" width="20" height="5" />
      <line x1="12" y1="22" x2="12" y2="7" />
      <path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7zM12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z" />
    </svg>
  ),
} as const

export type SummaryIconKey = keyof typeof icons

function TrendBadge({ trend }: { trend: { pct: number; direction: TrendDirection } | null }) {
  if (!trend) {
    return <span className="dashboard-kpi-trend dashboard-kpi-trend-neutral">—</span>
  }
  const { pct, direction } = trend
  if (direction === 'flat') {
    return <span className="dashboard-kpi-trend dashboard-kpi-trend-neutral">Flat</span>
  }
  const arrow = direction === 'up' ? '↑' : '↓'
  const modifier = direction === 'up' ? 'dashboard-kpi-trend-up' : 'dashboard-kpi-trend-down'
  return (
    <span className={`dashboard-kpi-trend ${modifier}`} title="Vs prior month">
      {arrow} {pct}%
    </span>
  )
}

export function DashboardSummaryCard({
  label,
  value,
  hint,
  icon,
  trend,
  primary,
}: {
  label: string
  value: string | number
  hint?: string
  icon: SummaryIconKey
  trend: { pct: number; direction: TrendDirection } | null
  primary?: boolean
}) {
  return (
    <article className={`dashboard-kpi-card${primary ? ' dashboard-kpi-primary' : ''}`}>
      <div className="dashboard-kpi-card-top">
        <span className="dashboard-kpi-icon" aria-hidden>
          {icons[icon]}
        </span>
        <TrendBadge trend={trend} />
      </div>
      <span>{label}</span>
      <strong>{value}</strong>
      {hint ? <p>{hint}</p> : null}
    </article>
  )
}
