...
type Props = {
  ...
  icon: ReactNode
  trend?: SummaryTrend
  /** Shown under the value when no trend hint */
  subtitle?: string
  primary?: boolean
}

export function DashboardSummaryCard({ label, value, icon, trend, subtitle, primary }: Props) {
    ...
    <article className={`dashboard-summary-card${primary ? ' dashboard-summary-card--primary' : ''}`}>
      ...
      {trend ? (
        <div className="dashboard-summary-card-trend">
          <span
            className={`dashboard-summary-trend-badge dashboard-summary-trend-badge--${trend.direction}${
              trend.favorable ? ' dashboard-summary-trend-badge--favorable' : ' dashboard-summary-trend-badge--caution'
            }`}
          >
          ...
          {trend.hint ? <span className="dashboard-summary-card-hint">{trend.hint}</span> : null}
        </div>
      ) : subtitle ? (
        <p className="dashboard-summary-card-subtitle">{subtitle}</p>
      ) : null}
    </article>
  )
}
