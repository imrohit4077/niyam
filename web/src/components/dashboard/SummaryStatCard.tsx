...
function TrendBadge({ direction, percent }: { direction: TrendDirection; percent: number }) {
  ...
  const label = direction === 'flat' && abs === 0 ? '0%' : `${arrow} ${abs}%`
  return <span className={`dashboard-summary-trend dashboard-summary-trend-${direction}`}>{label}</span>
}
  ...
  return (
    <article className={`dashboard-summary-card${primary ? ' dashboard-summary-card-primary' : ''}`}>
      <div className="dashboard-summary-card-top">
        ...
        {trend ? <TrendBadge direction={trend.direction} percent={trend.percent} /> : null}
      </div>
