...
const DASHBOARD_CHART_COLORS = ['#0ea5e9', '#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#6b7280', '#8b5cf6', '#6366f1']

const FUNNEL_STAGES = ['applied', 'screening', 'interview', 'offer', 'hired'] as const

const BRAND_LINE = '#0284c7'
...
function ChartSkeleton({ short }: { short?: boolean }) {
  return <div className={`dashboard-chart-skeleton ${short ? 'dashboard-chart-skeleton--short' : ''}`} aria-hidden />
}
...
function countByCreatedInRange(applications: Application[], start: Date, end: Date) {
  return applications.filter(a => isInRange(a.created_at, start, end)).length
}

function countOffersTouchedInRange(applications: Application[], start: Date, end: Date) {
  return applications.filter(a => a.status === 'offer' && isInRange(a.updated_at, start, end)).length
}

function countInterviewsActivityInRange(rows: InterviewAssignmentRow[], start: Date, end: Date) {
  return rows.filter(r => isInRange(r.created_at, start, end)).length
}

function countNewJobsInRange(jobList: Job[], start: Date, end: Date) {
  return jobList.filter(j => isInRange(j.created_at, start, end)).length
}

function dominantApplicationStage(applications: Application[], jobId: number): string | null {
...
function auditActivityLabel(entry: AuditLogEntry): string {
  const action = (entry.action || '').replace(/_/g, ' ').trim()
  const resource = (entry.resource || '').replace(/_/g, ' ').trim()
  const method = (entry.http_method || '').toUpperCase()
  if (action) return action.replace(/\b\w/g, c => c.toUpperCase())
  if (resource && method) return `${method} ${resource}`
  if (entry.path) {
    const tail = entry.path.replace(/^\/api\/v\d+\//, '')
    return `${method || 'Event'} ${tail}`.slice(0, 72)
  }
...
export default function HomeDashboardPage() {
  ...
  useEffect(() => {
    ...
    getAuditLog(token, { page: 1, per_page: 10 })
      .then(res => {
  ...
  const { current: winCur, previous: winPrev } = useMemo(() => rolling30DayWindows(), [])
  const summaryTrends = useMemo(() => {
    const cApps = countByCreatedInRange(allApplications, winCur.start, winCur.end)
    const pApps = countByCreatedInRange(allApplications, winPrev.start, winPrev.end)
    const cOffers = countOffersTouchedInRange(allApplications, winCur.start, winCur.end)
    const pOffers = countOffersTouchedInRange(allApplications, winPrev.start, winPrev.end)
    const cIv = countInterviewsActivityInRange(interviews, winCur.start, winCur.end)
    const pIv = countInterviewsActivityInRange(interviews, winPrev.start, winPrev.end)
    const cNewJobs = countNewJobsInRange(jobs, winCur.start, winCur.end)
    const pNewJobs = countNewJobsInRange(jobs, winPrev.start, winPrev.end)
    return {
      candidates: computePeriodTrend(cApps, pApps),
      jobs: computePeriodTrend(cNewJobs, pNewJobs),
      interviews: computePeriodTrend(cIv, pIv),
      offers: computePeriodTrend(cOffers, pOffers),
    }
  }, [allApplications, interviews, jobs, winCur.end, winCur.start, winPrev.end, winPrev.start])

  const funnelStageCounts = useMemo(() => {
    return FUNNEL_STAGES.map(stage => ({
      stage,
      label: formatDashboardLabel(stage),
      value: allApplications.filter(a => a.status === stage).length,
    }))
  }, [allApplications])

  const funnelMax = Math.max(...funnelStageCounts.map(f => f.value), 1)
  ...
  const jobDistribution = useMemo(() => {
    ...
    const truncate = (t: string) => (t.length > 28 ? `${t.slice(0, 26)}…` : t)
    const rows = jobs
      .map(j => ({ id: j.id, title: truncate(j.title), count: byJob[j.id] ?? 0 }))
      .filter(r => r.count > 0)
      .sort((a, b) => b.count - a.count)
      .slice(0, 8)
    return rows.length > 0 ? rows : jobs.slice(0, 8).map(j => ({ id: j.id, title: truncate(j.title), count: byJob[j.id] ?? 0 }))
  ...
  const funnelChartOptions: ChartOptions<'bar'> = useMemo(
    () => ({
      responsive: true,
      maintainAspectRatio: false,
      ...
      scales: {
        x: {
          beginAtZero: true,
          suggestedMax: funnelMax + Math.max(1, Math.ceil(funnelMax * 0.15)),
          ticks: { color: '#6b7280', precision: 0, font: { size: 11 } },
          grid: { color: 'rgba(148,163,184,0.22)' },
        },
        y: {
    ...
    <>
      ...
      <div className="dashboard-hero dashboard-hero-modern">
        <div className="dashboard-hero-main">
          ...
          <p className="dashboard-subtitle">
            Pipeline velocity, candidate volume, and hiring outcomes in one calm view—optimized for daily recruiting standups.
          </p>
        </div>
      ...
      <div className="dashboard-summary-grid">
        ...
        <DashboardSummaryCard
          ...
          hint={`${openJobs} open · ${jobs.length} total roles`}
        />
        <DashboardSummaryCard
          loading={summaryLoading}
      ...
      <div className="dashboard-grid dashboard-grid--charts">
        <DashboardPanel title="Candidates Pipeline (Workspace)">
          <div className="dashboard-panel-content">
            {applicationsLoading ? (
              <ChartSkeleton short />
            ) : funnelStageCounts.every(f => f.value === 0) ? (
              <div className="dashboard-empty">No candidates in pipeline stages yet.</div>
            ) : (
              <div className="dashboard-chart-shell dashboard-chart-shell-funnel">
                <Bar
                  data={{
                    labels: funnelStageCounts.map(f => f.label),
                    datasets: [
                      {
                        data: funnelStageCounts.map(f => f.value),
                    ...
                    datasets: [
                      {
                        ...
                        backgroundColor: jobDistribution.map(
                          (_, i) => DASHBOARD_CHART_COLORS[i % DASHBOARD_CHART_COLORS.length],
                        ),
                        borderRadius: 6,
                        maxBarThickness: 22,
      ...
      <div className="dashboard-grid dashboard-grid--split">
        <DashboardPanel title="Recent Activity">
          <div className="dashboard-panel-content">
            {auditLoading ? (
              <div className="dashboard-activity-skeletons" aria-hidden>
                {Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="dashboard-activity-skeleton-row">
                    ...
                    <div>
                      ...
                      <span className="dashboard-skeleton dashboard-skeleton--line-xs" />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <ul className="dashboard-activity-feed">
                {auditEntries.map(entry => (
                  <li key={entry.id} className="dashboard-activity-item">
                    ...
                    <div>
                      <p className="dashboard-activity-title">{auditActivityLabel(entry)}</p>
              ...
              <div className="dashboard-table-skeleton" aria-hidden>
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="dashboard-table-skeleton-row">
                    <span className="dashboard-skeleton dashboard-skeleton--line-md" />
                    <span className="dashboard-skeleton dashboard-skeleton--pill" />
                    <span className="dashboard-skeleton dashboard-skeleton--pill-sm" />
                    <span className="dashboard-skeleton dashboard-skeleton--pill-sm" />
                  </div>
                ))}
              </div>
                ...
                <table className="dashboard-jobs-table">
                  ...
                  <tbody>
                    {jobsTableRows.map(job => {
                      ...
                      const stage = dominantApplicationStage(allApplications, job.id)
                      return (
                        <tr key={job.id}>
                          <td>
