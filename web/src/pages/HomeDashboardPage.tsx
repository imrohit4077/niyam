...
import { DASHBOARD_BRAND_FILL, DASHBOARD_BRAND_LINE, DASHBOARD_CHART_COLORS } from '../components/dashboard/dashboardConstants'
import { DashboardKpiCard } from '../components/dashboard/DashboardKpiCard'
import { DashboardPageSkeleton, DashboardPanelSkeleton } from '../components/dashboard/DashboardSkeleton'
import {
  applicationsTrendInWindows,
  buildActivityFeed,
...
function DashboardPanel({ title, children, spanFull }: { title: string; children: ReactNode; spanFull?: boolean }) {
  return (
    <section
      className={`panel dashboard-panel dashboard-modern-panel${spanFull ? ' dashboard-panel--full' : ''}`}
    >
...
export default function HomeDashboardPage() {
  ...
  const funnelData = useMemo(() => funnelCountsFromApplications(allApplications), [allApplications])
  const jobDistribution = useMemo(() => applicantsPerJob(allApplications, jobs, 10), [allApplications, jobs])
  const jobById = useMemo(() => new Map(jobs.map(j => [j.id, j] as const)), [jobs])
  ...
  const candidatesTrend = useMemo(
    () => applicationsTrendInWindows(allApplications, a => a.created_at, 7),
    [allApplications],
  )
  const jobsOpenTrend = useMemo(() => openJobsTrend(jobs, 30), [jobs])
  const interviewsTrend = useMemo(() => interviewsScheduledTrend(interviews, 7), [interviews])
  const offersTrend = useMemo(() => offersReleasedTrend(allApplications, 7), [allApplications])
  ...
  const initialLoading = jobsLoading && applicationsLoading

  if (initialLoading && jobs.length === 0 && allApplications.length === 0) {
  ...
  return (
    <>
      ...
      <div className="dashboard-grid">
                    ...
                    datasets: [
                      {
                        ...
                        backgroundColor: funnelData.map(
        ...
        <DashboardPanel title="Application Trend (Last 6 Months)">
          <div className="dashboard-panel-content">
            ...
            <p className="dashboard-footnote dashboard-footnote--tight">
              Month-over-month delta on new applications: <strong>{monthlyDeltaLabel}</strong> ({currentMonthApplications} vs{' '}
        ...
        <DashboardPanel title="Workspace Sources (Overview)">
          <div className="dashboard-panel-content">
            {applicationsLoading ? (
              <DashboardPanelSkeleton />
        ...
        <DashboardPanel title="Jobs Overview" spanFull>
          <div className="dashboard-panel-content dashboard-panel-content--flush">
            {jobsLoading ? (
              ...
              <div className="dashboard-empty">No jobs yet. Create a role to see it here.</div>
            ) : (
                    ...
                    {jobs.map(job => (
                      <tr key={job.id}>
                        ...
                        <td className="dashboard-table-num">
                          {allApplications.filter(a => a.job_id === job.id).length}
                        </td>
