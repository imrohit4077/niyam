...
function trendFromCounts(current: number, previous: number, caption: string): KpiTrend | null {
  if (previous === 0 && current === 0) return null
  if (previous === 0) return { direction: 'up', percent: current > 0 ? 100 : 0, caption }
  const raw = Math.round(((current - previous) / previous) * 100)
  if (raw === 0) return { direction: 'flat', percent: 0, caption }
  return {
...
function countApplicationsInMonth(
  applications: Application[],
  year: number,
  monthIndex: number,
  field: 'created_at' | 'updated_at',
  predicate?: (a: Application) => boolean,
) {
...
function countJobsCreatedInMonth(jobs: Job[], year: number, monthIndex: number) {
...
function countInterviewsTouchingMonth(rows: InterviewAssignmentRow[], year: number, monthIndex: number) {
...
function buildActivityFeed(
  ...
  applications.forEach(a => {
    ...
    if (Array.isArray(a.stage_history) && a.stage_history.length > 0) {
      const last = a.stage_history[a.stage_history.length - 1]
      items.push({
        id: `app-stage-${a.id}-${last.changed_at}`,
        at: new Date(last.changed_at),
...
export default function HomeDashboardPage() {
  ...
  const applicantsThisMonth = countApplicationsInMonth(allApplications, curY, curM, 'created_at')
  const applicantsPrevMonth = countApplicationsInMonth(allApplications, prevY, prevM, 'created_at')
  const offersThisMonth = countApplicationsInMonth(allApplications, curY, curM, 'updated_at', a => a.status === 'offer')
  const offersPrevMonth = countApplicationsInMonth(allApplications, prevY, prevM, 'updated_at', a => a.status === 'offer')
  const newJobListingsThisMonth = countJobsCreatedInMonth(jobs, curY, curM)
  const newJobListingsPrevMonth = countJobsCreatedInMonth(jobs, prevY, prevM)
  const interviewsThisMonth = countInterviewsTouchingMonth(interviews, curY, curM)
  const interviewsPrevMonth = countInterviewsTouchingMonth(interviews, prevY, prevM)

  const funnelCounts = useMemo(() => {
    return FUNNEL_STAGES.map(stage =>
      allApplications.reduce((n, a) => n + (a.status === stage ? 1 : 0), 0),
    )
  }, [allApplications])
  ...
  const jobDistribution = useMemo(() => {
    ...
    if (rest > 0) top.push({ jobId: -1, count: rest, title: 'Other roles' })
    return top
    ...
    <>
      ...
      <div className="dashboard-hero dashboard-hero-modern">
        <div className="dashboard-hero-main">
          ...
          <p className="dashboard-subtitle">
            Pipeline velocity, hiring funnel health, and team activity — scoped to your workspace, with role-level detail when you pick a job.
          </p>
        </div>
      ...
      <div className="dashboard-kpi-grid dashboard-kpi-grid--summary">
        <DashboardKpiCard
          ...
          icon={<IconUsers />}
          trend={trendFromCounts(applicantsThisMonth, applicantsPrevMonth, 'vs last month')}
          sublabel={`${applicantsThisMonth} new applications this month`}
          primary
        ...
        <DashboardKpiCard
          ...
          value={openJobs}
          icon={<IconBriefcase />}
          trend={trendFromCounts(newJobListingsThisMonth, newJobListingsPrevMonth, 'new roles listed')}
        ...
        <DashboardKpiCard
          ...
          icon={<IconCalendar />}
          trend={trendFromCounts(interviewsThisMonth, interviewsPrevMonth, 'touching this month vs last')}
        ...
        <DashboardKpiCard
          ...
          icon={<IconGift />}
          trend={trendFromCounts(offersThisMonth, offersPrevMonth, 'in offer stage (updates)')}
          sublabel="Workspace-wide in offer stage"
          loading={summaryLoading}
        />
      </div>

      <div className="dashboard-charts-grid">
        <DashboardPanel title="Candidates pipeline" className="dashboard-panel-span-6">
        ...
        <DashboardPanel
          ...
          className="dashboard-panel-span-6"
          actions={
            selectedJob ? (
              <span className="dashboard-panel-action-meta" title={selectedJob.title}>
                {selectedJob.title}
              </span>
            ) : null
          }
        >
          <div className="dashboard-panel-content">
            {analyticsLoading ? (
              ...
              <div className="dashboard-empty">No source data for the selected job.</div>
            ) : (
              <>
                <div className="dashboard-chart-shell dashboard-chart-shell-short">
                  <Pie
          ...
          <div className="dashboard-panel-content">
            {jobsLoading ? (
              ...
              <>
                ...
                <div className="dashboard-footnote">Tip: select a role to update pipeline detail, sources, and the jobs table context.</div>
              </>
            )}
          </div>
        </DashboardPanel>
        ...
        <DashboardPanel title="Recent activity">
          <div className="dashboard-panel-content">
            {applicationsLoading ? (
              ...
              <div className="dashboard-empty">Activity will appear as candidates apply and interviews move.</div>
            ) : (
              <ul className="dashboard-activity-feed">
                {activityItems.map(item => (
                  <li key={item.id} className="dashboard-activity-item">
                    ...
                    <time className="dashboard-activity-time" dateTime={item.at.toISOString()}>
                      {item.at.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                    </time>
                  </li>
                ))}
              </ul>
              ...
              <div className="dashboard-table-skeleton-wrap" aria-busy="true">
                {Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="dashboard-table-skeleton-row">
                    <span className="dashboard-skeleton dashboard-skeleton--table-cell-lg" />
                    <span className="dashboard-skeleton dashboard-skeleton--table-cell-sm" />
                    <span className="dashboard-skeleton dashboard-skeleton--table-cell-sm" />
                    <span className="dashboard-skeleton dashboard-skeleton--table-cell-md" />
                  </div>
                ))}
              </div>
            ) : jobsError ? (
                ...
                <table className="dashboard-jobs-table">
                  ...
                  <tbody>
                    {jobs.map(job => {
                      ...
                      const focusStage = (['interview', 'offer', 'screening', 'applied', 'hired'] as const).find(s => (bySt[s] ?? 0) > 0)
                      const stageLabel = focusStage ? primaryStageLabel(focusStage) : '—'
                      return (
                        <tr key={job.id} className={selectedJobId === String(job.id) ? 'dashboard-jobs-table-row-active' : undefined}>
                          <td>
